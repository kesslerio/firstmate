...
effort_flag_for_harness() {
  ...
  codex)
    ...
    # installed model catalog advertises max for gpt-6-luna; keep that level
    # keyed to that one model rather than to every catalog entry that
    # advertises it.
    case "$effort" in
    low | medium | high | xhigh) printf -- '-c %s ' "$(shell_quote "model_reasoning_effort=\"$effort\"")" ;;
    max)
      [ "$model" = gpt-6-luna ] || return 0
